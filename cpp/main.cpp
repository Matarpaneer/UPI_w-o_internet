...
int main() {
    ...
    auto settlement = std::make_shared<service::SettlementService>(accRepo, txRepo);
    auto bridge = std::make_shared<service::BridgeIngestionService>(cryptoSvc, idempotency, settlement);
    ...
    drogon::app().registerPostHandlingAdvice([](const drogon::HttpRequestPtr& req,
        ...
        resp->addHeader("Access-Control-Allow-Headers", "Content-Type, X-Bridge-Node-Id, X-Hop-Count");
    });
