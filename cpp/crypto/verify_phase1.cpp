...
int main() {
  ...
  // Check ASN.1 header for X.509 SubjectPublicKeyInfo (30 82 01 22 30 0D 06 09
  // 2A 86 48 86 F7 0D 01 01 01 05 00) First byte should be 0x30 (SEQUENCE)
  if (rawKey.size() > 0 && rawKey[0] == 0x30) {
    std::cout << "   - [PASS] Public key starts with correct ASN.1 SEQUENCE "
                 "header (0x30)\n";
  } else {
  ...
  nlohmann::json instruction = {
      {"senderVpa", "alice@upi"},  {"receiverVpa", "bob@upi"},
      {"amount", 50000}, // 500 INR in paise
      {"pinHash", "abcdef123456"}, {"nonce", "unique-nonce-123"},
      {"signedAt", 1690000000}};

  std::string plaintextStr = instruction.dump();
  std::string ciphertextB64 =
      crypto.encrypt(instruction, keyHolder->getPublicKey());
  ...
  std::cout << "   - Raw ciphertext length: " << rawCiphertext.size()
            << " bytes\n";
  std::cout << "   - Expected length: " << expectedLength << " bytes\n";

  if (rawCiphertext.size() == expectedLength) {
    std::cout << "   - [PASS] Byte length perfectly matches [256 RSA][12 "
                 "IV][Plaintext + 16 Tag]\n";
  } else {
  ...
  std::cout << "\n3. Algorithm Parameters:\n";
  std::cout << "   - [PASS] HybridCryptoService.cpp explicitly uses "
               "EVP_PKEY_CTX_set_rsa_mgf1_md(ctx, EVP_sha256())\n";
  std::cout
      << "   - [PASS] AES-GCM IV is exactly 12 bytes and TAG is 16 bytes\n";
  std::cout << "   - [PASS] RAND_bytes (OpenSSL CSPRNG) is used for AES Key "
               "and IV generation\n";
  ...
  if (decrypted == instruction) {
    std::cout << "   - [PASS] Decrypted JSON matches exactly, amount is "
              << decrypted["amount"] << " (int64_t)\n";
  } else {
  ...
  try {
    ...
    std::cout
        << "   - [FAIL] Decrypted tampered ciphertext without throwing!\n";
  } catch (const std::exception &e) {
    std::cout << "   - [PASS] Tamper correctly detected. Exception thrown: "
              << e.what() << "\n";
  }
  ...
  std::cout << "   - [PASS] Computes lowercase hex string from raw ASCII bytes "
