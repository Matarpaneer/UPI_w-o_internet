...
HybridCryptoService::HybridCryptoService(
    std::shared_ptr<ServerKeyHolder> serverKey)
    : serverKey_(std::move(serverKey)) {}

std::string HybridCryptoService::encrypt(const nlohmann::json &instruction,
  ...
  if (EVP_EncryptInit_ex(aesCtx, EVP_aes_256_gcm(), nullptr, nullptr,
                         nullptr) != 1) {
    EVP_CIPHER_CTX_free(aesCtx);
    throw std::runtime_error("Failed to init AES-GCM");
  }

  if (EVP_EncryptInit_ex(aesCtx, nullptr, nullptr, aesKey.data(), iv.data()) !=
      1) {
    EVP_CIPHER_CTX_free(aesCtx);
  ...
  if (EVP_CIPHER_CTX_ctrl(aesCtx, EVP_CTRL_GCM_GET_TAG, GCM_TAG_BYTES,
                          tag.data()) != 1) {
    EVP_CIPHER_CTX_free(aesCtx);
  ...
  size_t outlen;
  if (EVP_PKEY_encrypt(rsaCtx, nullptr, &outlen, aesKey.data(),
                       aesKey.size()) <= 0) {
    EVP_PKEY_CTX_free(rsaCtx);
  ...
  if (EVP_PKEY_encrypt(rsaCtx, rsaCiphertext.data(), &outlen, aesKey.data(),
                       aesKey.size()) <= 0) {
    EVP_PKEY_CTX_free(rsaCtx);
  ...
  packed.reserve(rsaCiphertext.size() + iv.size() + ciphertext.size() +
                 tag.size());
...
nlohmann::json
HybridCryptoService::decrypt(const std::string &base64Ciphertext) {
  ...
  size_t aesKeyLen;
  if (EVP_PKEY_decrypt(rsaCtx, nullptr, &aesKeyLen, rsaCiphertext.data(),
                       rsaCiphertext.size()) <= 0) {
    EVP_PKEY_CTX_free(rsaCtx);
    throw std::runtime_error(
        "Failed to determine AES key buffer size (decryption failure)");
  }

  std::vector<unsigned char> aesKey(aesKeyLen);
  if (EVP_PKEY_decrypt(rsaCtx, aesKey.data(), &aesKeyLen, rsaCiphertext.data(),
                       rsaCiphertext.size()) <= 0) {
    EVP_PKEY_CTX_free(rsaCtx);
  ...
  if (EVP_DecryptInit_ex(aesCtx, EVP_aes_256_gcm(), nullptr, nullptr,
                         nullptr) != 1) {
    EVP_CIPHER_CTX_free(aesCtx);
    throw std::runtime_error("Failed to init AES-GCM decrypt");
  }

  if (EVP_DecryptInit_ex(aesCtx, nullptr, nullptr, aesKey.data(), iv.data()) !=
      1) {
    EVP_CIPHER_CTX_free(aesCtx);
    throw std::runtime_error("Failed to set AES key and IV");
  }
  ...
  int len;
  if (EVP_DecryptUpdate(aesCtx, plaintext.data(), &len, aesCiphertext.data(),
                        aesCiphertext.size()) != 1) {
    EVP_CIPHER_CTX_free(aesCtx);
  ...
  if (EVP_CIPHER_CTX_ctrl(aesCtx, EVP_CTRL_GCM_SET_TAG, GCM_TAG_BYTES,
                          tag.data()) != 1) {
    EVP_CIPHER_CTX_free(aesCtx);
  ...
  if (ret <= 0) {
    throw std::runtime_error(
        "AES-GCM Authentication failed (tampered ciphertext or tag)");
  }
  plaintextLen += len;
...
std::string
HybridCryptoService::base64Encode(const std::vector<unsigned char> &data) {
...
HybridCryptoService::base64Decode(const std::string &b64) {
...
std::string
HybridCryptoService::hashCiphertext(const std::string &base64Ciphertext) {
  EVP_MD_CTX *ctx = EVP_MD_CTX_new();
  if (!ctx)
    throw std::runtime_error("Failed to create MD ctx");

  if (EVP_DigestInit_ex(ctx, EVP_sha256(), nullptr) != 1 ||
      EVP_DigestUpdate(ctx, base64Ciphertext.c_str(),
                       base64Ciphertext.length()) != 1) {
    EVP_MD_CTX_free(ctx);
