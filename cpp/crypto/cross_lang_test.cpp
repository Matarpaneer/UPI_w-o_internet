...
int main() {
  ...
  std::string curlCmd1 =
      "curl -s -X POST -H \"Content-Type: text/plain\" -d \"" + cppPub +
      "\" http://localhost:8080/api/demo/encrypt-test";
  ...
  std::string javaPubStr =
      execCurl("curl -s http://localhost:8080/api/server-key");
  ...
  nlohmann::json payload = {{"senderVpa", "alice@upi"},
                            {"receiverVpa", "bob@upi"},
                            {"amount", 50000},
  ...
  std::cout << "Sending C++ Ciphertext (length " << cppCiphertext.length()
            << ") to Java for decryption...\n";
  std::string curlCmd2 =
      "curl -s -X POST -H \"Content-Type: text/plain\" -d \"" + cppCiphertext +
      "\" http://localhost:8080/api/demo/decrypt-test";
