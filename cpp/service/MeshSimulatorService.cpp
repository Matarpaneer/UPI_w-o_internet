...
void MeshSimulatorService::seedDefaultDevices() {
    devices_["phone-alice"] = std::make_shared<VirtualDevice>("phone-alice", false);
    devices_["phone-stranger1"] = std::make_shared<VirtualDevice>("phone-stranger1", false);
    devices_["phone-stranger2"] = std::make_shared<VirtualDevice>("phone-stranger2", false);
    devices_["phone-stranger3"] = std::make_shared<VirtualDevice>("phone-stranger3", false);
    devices_["phone-bridge"] = std::make_shared<VirtualDevice>("phone-bridge", true);
}

std::vector<std::shared_ptr<VirtualDevice>> MeshSimulatorService::getDevices() const {
    std::vector<std::shared_ptr<VirtualDevice>> result;
    for (const auto& [id, dev] : devices_) result.push_back(dev);
    return result;
}

std::shared_ptr<VirtualDevice> MeshSimulatorService::getDevice(const std::string& id) const {
    auto it = devices_.find(id);
    return it != devices_.end() ? it->second : nullptr;
}

void MeshSimulatorService::inject(const std::string& senderDeviceId, const model::MeshPacket& packet) {
    auto sender = getDevice(senderDeviceId);
    if (!sender) throw std::invalid_argument("Unknown device: " + senderDeviceId);
    sender->hold(packet);
    std::cout << "Packet " << packet.packetId.substr(0, 8) 
              << " injected at " << senderDeviceId << " (TTL=" << packet.ttl << ")\n";
}
