...
void testSettlementConcurrency() {
  std::cout << "\n2. Settlement Optimistic Lock Retry Test (10 concurrent Txns "
               "on same account)\n";
  ...
  std::atomic<int> exhaustedRetries{0};
  ...
  std::cout << "   - Exhausted Retries (Failed completely): "
            << exhaustedRetries << "\n";
  std::cout << "   - Alice balance delta: "
            << (initialAliceBalance - finalAliceBalance)
            << " (Expected 1000)\n";
  std::cout << "   - Bob balance delta: "
            << (finalBobBalance - initialBobBalance) << " (Expected 1000)\n";

  if (completedTxns == 10 &&
      (initialAliceBalance - finalAliceBalance == 1000)) {
    std::cout << "   - [PASS] Bounded retry successfully bypassed optimistic "
                 "lock races!\n";
  } else {
