...
public:
    ...
    // Throws a runtime_error (constraint violation) if the packetHash already exists.
    int64_t save(Transaction& tx);

    // Defense-in-depth: Checks if a transaction with the given packetHash already exists.
