...
int main() {
  ...
  if (alice && alice->balance == 500000) {
    std::cout << "   - [PASS] Database initialized and alice@demo found with "
                 "balance 500000 paise (Rs 5000.00)\n";
  } else {
  ...
  std::cout << "   - Read bob@demo (version " << bob.version << ", balance "
            << bob.balance << ")\n";
  ...
  accRepo.save(bob);
  std::cout
      << "   - Saved bob legitimately. New version in DB is expected to be "
      << bob.version + 1 << "\n";
  ...
  try {
    ...
    std::cout << "   - [FAIL] Successfully saved a stale view. Optimistic "
                 "locking failed!\n";
  } catch (const ConcurrencyException &e) {
    std::cout << "   - [PASS] Stale update rejected correctly. Exception: "
              << e.what() << "\n";
  }
  ...
  tx2.amount =
      99999; // Change amount just to prove the DB rejects purely on hash
  try {
    txRepo.save(tx2);
    std::cout << "   - [FAIL] DB accepted a transaction with a duplicate "
                 "packetHash!\n";
  } catch (const std::exception &e) {
    std::cout
        << "   - [PASS] Duplicate hash rejected by DB constraint. Exception: "
        << e.what() << "\n";
  }

  bool exists = txRepo.existsByPacketHash("abcdef1234567890_mock_hash");
  std::cout << "   - [PASS] existsByPacketHash returned "
            << (exists ? "true" : "false") << " (expected true)\n";
