...
int64_t TransactionRepository::save(Transaction& tx) {
    ...
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) != SQLITE_OK) {
        throw std::runtime_error(std::string("Failed to prepare tx save statement: ") + sqlite3_errmsg(db));
    }
...
std::vector<Transaction> TransactionRepository::findTop20ByOrderByIdDesc() {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        tx.packetHash = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1));
        tx.senderVpa = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2));
        tx.receiverVpa = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3));
        tx.amount = sqlite3_column_int64(stmt, 4);
        tx.signedAt = sqlite3_column_int64(stmt, 5);
        tx.settledAt = sqlite3_column_int64(stmt, 6);
        tx.bridgeNodeId = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 7));
        tx.hopCount = sqlite3_column_int(stmt, 8);
        tx.status = parseStatus(reinterpret_cast<const char*>(sqlite3_column_text(stmt, 9)));
