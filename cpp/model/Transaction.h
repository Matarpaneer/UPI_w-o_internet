...
enum class Status { SETTLED, REJECTED, DUPLICATE_DROPPED, INVALID };
...
inline void to_json(nlohmann::json &j, const model::Transaction &t) {
  j = nlohmann::json{{"id", t.id},
                     {"packetHash", t.packetHash},
                     {"senderVpa", t.senderVpa},
                     {"receiverVpa", t.receiverVpa},
                     {"amount", static_cast<double>(t.amount) / 100.0},
                     {"signedAt", t.signedAt},
                     {"settledAt", t.settledAt},
                     {"bridgeNodeId", t.bridgeNodeId},
                     {"hopCount", t.hopCount},
                     {"status", model::statusToString(t.status)}};
}
